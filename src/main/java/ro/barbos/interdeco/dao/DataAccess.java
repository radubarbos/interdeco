...
public class DataAccess
{
    ...
    private String serverURL="jdbc:mysql://127.0.0.1:3306/interdeco?autoReconnect=true";
    ...
    public Connection getDatabaseConnection(boolean commit){
        if(con !=null) {
            try{
        ...
        try{
            ...
            if(con!=null && !commit) con.setAutoCommit(false);
        }catch (Exception e) {
            con=null;
            e.printStackTrace();
        }
    ...
    public void init(){
        ...
        if(customDBServer!=null && customDBServer.trim().length()>0)
        {
            ...
            serverURL="jdbc:mysql://"+server+":3306/interdeco?autoReconnect=true";
        }
        try
        {
            Class.forName(driver);
